...
<button type="button" class="btn btn-success" id="Aja">Aja</button>
<button type="button" class="btn btn-danger" id="Doc">Doc</button>
<button type="button" class="btn btn-warning" id="Weeee">Weeee</button>
`;

  renderToDom("#buttonContainer", domString);
};

//this gives access to the event, it gets the ID is why it is important!!
const handleButtonClick = (event) => {
  if (event.target.id === "All") {
    console.log("The All Button");
  }
  ...
  if (event.target.id === "Weeee") {
    console.log("Weeee's Button");
  }
};
...
const init = () => {
  //this starts the app
  buttons(); //Puit DOM ELEMENTS FIRST
